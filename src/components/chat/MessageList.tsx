...
export function MessageList({ messages, isLoading }: MessageListProps) {
  if (messages.length === 0) {
    return (
      <div className="absolute inset-0 flex flex-col items-center justify-end px-4 pb-8 text-center">
                              ...
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs border border-neutral-200">
                                {tool.state === "result" && tool.result ? (
                                  <>
                                    ...
                                    <span className="text-neutral-700">{formatToolCall(tool.toolName, tool.args)}</span>
                                  </>
                                ) : (
                                  <>
